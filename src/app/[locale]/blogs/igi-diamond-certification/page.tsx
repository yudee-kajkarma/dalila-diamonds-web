...
import { getBlog7Data, BLOG7_PATH } from "@/lib/i18n/getBlog7Data";
...
export async function generateMetadata({
  ...
  const canonicalUrl = `${baseUrl}${getLocalizedPath(BLOG7_PATH, locale)}`;

  const languageAlternates: Record<string, string> = {};
  LOCALES.forEach((loc) => {
    languageAlternates[loc] = `${baseUrl}${getLocalizedPath(BLOG7_PATH, loc)}`;
  });
  languageAlternates["x-default"] = `${baseUrl}${getLocalizedPath(BLOG7_PATH, DEFAULT_LOCALE)}`;
...
export default async function IgiDiamondCertificationLocalePage({
  params,
}: {
  params?: Promise<{ locale?: string }>;
}) {
  ...
  return <IgiDiamondCertification locale={locale} />;
}
