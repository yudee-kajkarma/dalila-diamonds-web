...
function buildStructuredDataGraph(data: FluorescenceGuidePageData, locale: Locale) {
...
interface DiamondFluorescenceGuideProps {
  locale?: Locale;
}

export default function DiamondFluorescenceGuide({ locale = "en" }: DiamondFluorescenceGuideProps) {
  const data = getFluorescenceGuideData(locale);
  ...
  return (
    <main className={`${marcellus.variable} ${jost.variable} bg-white min-h-screen`}>
      ...
      {/* Hero */}
      <section className="bg-white py-10 md:py-14" aria-labelledby="fluorescence-guide-hero-heading">
        <div className="container mx-auto max-w-7xl px-4">
          <AnimatedContainer direction="up">
            <h1
              id="fluorescence-guide-hero-heading"
      ...
      {/* Quick Answer */}
      <section className="bg-[#FAF6EB] border-y border-[#c89e3a]/20 py-10 md:py-12" aria-labelledby="fluorescence-guide-quick-answer">
        <div className="container mx-auto max-w-7xl px-4">
          <AnimatedContainer direction="up">
            <h2
              id="fluorescence-guide-quick-answer"
      ...
      <div className="container mx-auto max-w-7xl px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-12">
          <aside className="lg:w-56 w-full shrink-0">
            <AnimatedContainer direction="left">
              <div className="sticky top-4">
                <ResourceSidebar currentPage="diamond-fluorescence-guide" />
              </div>
            </AnimatedContainer>
          </aside>
          ...
          <article className="flex-1 w-full min-w-0">
            ...
            {renderSection({
              id: "fluorescence-guide-key-takeaways",
            ...
            <div className="mb-12">
              <AnimatedContainer direction="up">
                <nav className="bg-white scroll-mt-28" aria-labelledby="fluorescence-guide-overview-nav">
                  <div className="w-24 h-1.5 bg-linear-to-r from-[#c89e3a] to-[#e4c75f] mb-6 rounded-full" />
                  <h2
                    id="fluorescence-guide-overview-nav"
            ...
            {renderSection({
              id: data.whyFluoresce.id,
              title: data.whyFluoresce.title,
              paragraphsBefore: data.whyFluoresce.introParagraphs,
              bullets: data.whyFluoresce.bullets,
              paragraphsAfter: data.whyFluoresce.closingParagraphs,
            })}

            {renderSection({
              id: data.fluorescenceScale.id,
              title: data.fluorescenceScale.title,
              paragraphsBefore: data.fluorescenceScale.introParagraphs,
              table: data.fluorescenceScale.table,
              paragraphsAfter: data.fluorescenceScale.closingParagraphs,
              children: (
                <>
                  <RichParagraphList
                    paragraphs={data.fluorescenceScale.richClosingParagraphs as RichSegment[][]}
                    locale={locale}
                  />
                  {data.fluorescenceScale.subsections.map((subsection) => renderSubsection(subsection, locale))}
                </>
              ),
            })}

            {renderSection({
              id: data.intensityVsColour.id,
              title: data.intensityVsColour.title,
              paragraphsBefore: data.intensityVsColour.introParagraphs,
              bullets: data.intensityVsColour.bullets,
              paragraphsAfter: data.intensityVsColour.closingParagraphs,
              children: (
                <>
                  <BulletList items={data.intensityVsColour.colourBullets} />
                  <PlainParagraphs items={data.intensityVsColour.finalParagraphs} />
                </>
              ),
            })}

            {renderSection({
              id: data.fluorescenceChart.id,
              title: data.fluorescenceChart.title,
              paragraphsBefore: data.fluorescenceChart.introParagraphs,
              table: data.fluorescenceChart.table,
              paragraphsAfter: data.fluorescenceChart.closingParagraphs,
            })}

            {renderSection({
              id: data.lightingAppearance.id,
              title: data.lightingAppearance.title,
              paragraphsBefore: data.lightingAppearance.introParagraphs,
            })}
            {data.lightingAppearance.subsections.map((subsection) => renderSubsection(subsection, locale))}

            {renderSection({
              id: data.blueAndColour.id,
              title: data.blueAndColour.title,
              paragraphsBefore: data.blueAndColour.introParagraphs,
            })}
            {data.blueAndColour.subsections.map((subsection) => renderSubsection(subsection, locale))}

            {renderSection({
              id: data.haziness.id,
              title: data.haziness.title,
              paragraphsBefore: data.haziness.introParagraphs,
              children: (
                <>
                  <RichParagraphList
                    paragraphs={data.haziness.richIntroParagraphs as RichSegment[][]}
                    locale={locale}
                  />
                  {data.haziness.subsections.map((subsection) => renderSubsection(subsection, locale))}
                </>
              ),
            })}

            {renderSection({
              id: data.sparkleAndCut.id,
              title: data.sparkleAndCut.title,
              paragraphsBefore: data.sparkleAndCut.introParagraphs,
              bullets: data.sparkleAndCut.bullets,
              paragraphsAfter: data.sparkleAndCut.closingParagraphs,
            })}
            {data.sparkleAndCut.subsections.map((subsection) => renderSubsection(subsection, locale))}

            {renderSection({
              id: data.shape.id,
              title: data.shape.title,
              paragraphsBefore: data.shape.introParagraphs,
            })}
            {data.shape.subsections.map((subsection) => renderSubsection(subsection, locale))}

            {renderSection({
              id: data.multiStone.id,
              title: data.multiStone.title,
              paragraphsBefore: data.multiStone.introParagraphs,
              bullets: data.multiStone.bullets,
              paragraphsAfter: data.multiStone.closingParagraphs,
            })}
            {data.multiStone.subsections.map((subsection) => renderSubsection(subsection, locale))}

            {renderSection({
              id: data.price.id,
              title: data.price.title,
              paragraphsBefore: data.price.introParagraphs,
              bullets: data.price.bullets,
            })}
            ...
            {renderSection({
              ...
              children: (
                <>
                  <RichParagraphList
                    paragraphs={data.naturalVsLab.richClosingParagraphs as RichSegment[][]}
                    locale={locale}
                  />
                  {data.naturalVsLab.subsections.map((subsection) => renderSubsection(subsection, locale))}
                </>
              ),
            })}

            {renderSection({
              id: data.vsPhosphorescence.id,
              title: data.vsPhosphorescence.title,
              paragraphsBefore: data.vsPhosphorescence.introParagraphs,
              table: data.vsPhosphorescence.table,
              paragraphsAfter: data.vsPhosphorescence.closingParagraphs,
            })}
      ...
      {/* FAQ */}
      <section className="bg-white py-12 md:py-16" aria-labelledby="fluorescence-guide-faq-heading">
        <div className="container mx-auto max-w-7xl px-4">
          <AnimatedContainer direction="up">
            <div className="w-24 h-1.5 bg-linear-to-r from-[#c89e3a] to-[#e4c75f] mb-6 rounded-full" />
            <h2
              id="fluorescence-guide-faq-heading"
      ...
      <section
        ...
        aria-labelledby="fluorescence-guide-final-takeaway-heading"
      >
        <div className="container mx-auto max-w-7xl px-4">
          <AnimatedContainer direction="up">
            <div className="w-24 h-1.5 bg-linear-to-r from-[#c89e3a] to-[#e4c75f] mb-6 rounded-full" />
            <h2
              id="fluorescence-guide-final-takeaway-heading"
