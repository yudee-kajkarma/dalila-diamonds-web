...
interface SydProcessProps {
  content: SydContent["process"];
}

export default function SydProcess({ content }: SydProcessProps) {
  return (
    <section
      id={content.id}
      className="bg-gray-50 py-12 md:py-16"
      aria-labelledby="syd-process-heading"
    >
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
        <div className="w-24 h-1.5 bg-gradient-to-r from-[#c89e3a] to-[#e4c75f] mb-6 rounded-full" />
        <h2
          id="syd-process-heading"
        ...
        <div className="space-y-12 md:space-y-16">
          {content.steps.map((step) => (
            <article key={step.tag} className="max-w-4xl">
              ...
              <div
                className={`space-y-4 text-gray-700 text-[15px] md:text-base leading-relaxed ${jost.className}`}
              >
                {step.paragraphs.map((paragraph, index) => (
                  <p key={`${step.tag}-${index}`}>{paragraph}</p>
                ))}
              </div>
            </article>
          ))}
