...
const en = JSON.parse(fs.readFileSync(path.join(base, "diamond-girdle-guide.json"), "utf8"));
...
async function runLocale(loc, tl) {
  const cachePath = path.join(base, `.girdle-cache-${loc}.json`);
  ...
  fs.writeFileSync(path.join(base, `diamond-girdle-guide.${loc}.json`), JSON.stringify(translated, null, 2) + "\n");
