...
const base = path.join(__dirname, "../src/data/blogs/blog3");
...
async function runLocale(loc, tl) {
  const cachePath = path.join(base, `.blog3-cache-${loc}.json`);
  ...
  translated.images = {
    ...
    valueMatrix: {
      ...en.images.valueMatrix,
      alt: translated.images.valueMatrix.alt,
    },
    byShape: {
      ...en.images.byShape,
      alt: translated.images.byShape.alt,
    },
    metalComparison: {
      ...en.images.metalComparison,
      alt: translated.images.metalComparison.alt,
    },
    logo: en.images.logo,
  };
